...
int robot_status::_encodeNoHash(void *buf, int offset, int maxlen) const
{
    ...
    tlen = __float_encode_array(buf, offset + pos, maxlen - pos, &this->bat, 1);
    if(tlen < 0) return tlen; else pos += tlen;
...
int robot_status::_decodeNoHash(const void *buf, int offset, int maxlen)
{
    ...
    tlen = __float_decode_array(buf, offset + pos, maxlen - pos, &this->bat, 1);
    if(tlen < 0) return tlen; else pos += tlen;
...
{
    uint64_t hash = 0xf530b03a071958a5LL;
